                ...
                let tool_result = match func_name {
                                    ...
                                    Ok(child) => {
                                        ...
                                        if let Some(seconds) = timer_opt {
                                            ...
                                            tokio::spawn(async move {
                                                ...
                                                crate::state::log_info(&format!(
                                                    "Timer de {}s expiró para PID {}",
                                                    seconds, pid_copy
                                                ));
                                            });
                                        }
                    ...
                    "image_release" => {
                        ...
                        if id.is_empty() {
                            ...
                            messages.retain(|msg| {
                                // Nuevo formato: content string (texto plano)
                                if let Some(text) = msg["content"].as_str() {
                                    if text.contains(&marker) { return false; }
                                }
                                // Formato antiguo: content array (multimodal)
                    ...
                    "git_resolve_divergence" => {
                        ...
                        match action {
                            "keep_local" => {
                                match Command::new("git").args(&["push", "origin", "master", "--force"])
                                    .current_dir(&proj_path).env("GIT_TERMINAL_PROMPT", "0").output()
                                {
                            ...
                            "keep_remote" => {
                                match Command::new("git").args(&["reset", "--hard", "origin/master"])
                                    .current_dir(&proj_path).env("GIT_TERMINAL_PROMPT", "0").output()
                                {
                                    Ok(o) if o.status.success() =>
                            ...
                            "merge_both" => {
                                match Command::new("git").args(&["pull", "--rebase", "--autostash", "origin", "master"])
                                    .current_dir(&proj_path).env("GIT_TERMINAL_PROMPT", "0").env("GIT_MERGE_AUTOEDIT", "no").output()
                                {
                                    Ok(o) if o.status.success() =>
                                    ...
                                    Ok(o) => {
                                        ...
                                        if stderr.contains("CONFLICT") || stderr.contains("conflict") {
                                            let _ = Command::new("git").args(&["rebase", "--abort"])
                                                .current_dir(&proj_path).env("GIT_TERMINAL_PROMPT", "0").status();
                                            ...
                                            match resp.json::<serde_json::Value>() {
                                                Ok(json_resp) => {
                                                    let content = json_resp["choices"][0]["message"]["content"]
                                                        .as_str().unwrap_or("(Sin respuesta)");
